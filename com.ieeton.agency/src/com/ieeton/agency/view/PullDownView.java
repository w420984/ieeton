...
public class PullDownView extends FrameLayout implements OnGestureListener, AnimationListener {
	...
	private void initArrow() {
        mUpArrow = getContext().getApplicationContext().getResources().getDrawable(R.drawable.refresh_list_release_up);
        mDownArrow = getContext().getApplicationContext().getResources().getDrawable(R.drawable.refresh_list_pull_down);
	}
	...
	public void initSkin(){
		...
		mTitle1.setTextColor(getResources().getColor(Color.BLACK));
		mTitle2.setTextColor(getResources().getColor(Color.GRAY));
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (iconWidth > 0 || mIsCheckLeftPadding) {
			int paddingLeft = (getMeasuredWidth() - ((int)(textWidth + iconWidth + getContext().getApplicationContext().getResources().getDimension(R.dimen.updatebar_icon_space)))) >> 1;
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		try{
			...
			if((retValue || mState == STATE_OPEN || mState == STATE_OPEN_MAX || mState == STATE_OPEN_MAX_RELEASE || mState == STATE_OPEN_RELEASE)&&getContentView().getTop() != 0){
				...
				try {
					istouch=super.dispatchTouchEvent(event);
