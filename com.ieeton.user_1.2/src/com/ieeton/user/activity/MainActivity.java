...
import com.ieeton.user.domain.InviteMessage;
import com.ieeton.user.domain.User;
import com.ieeton.user.domain.InviteMessage.InviteMesageStatus;
...
public class MainActivity extends FragmentActivity {
	...
	public static String INPUT_INDEX = "input_index";
	public static final int INPUT_HOME = 0;
	public static final int INPUT_DISCOVERY = 1;
	public static final int INPUT_CHAT_HISTORY = 2;
	public static final int INPUT_SETTING = 3;
	...
	private IeetonReceiver mIeetonReceiver;
	public class IeetonReceiver extends BroadcastReceiver{

		@Override
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			if(Constants.NEED_RELOGIN_ACTION.equals(action)){
...
//		registerReceiver(offlineMessageReceiver, offlineMessageIntentFilter);
	...
	private void showDefaultTab(int index){
		FragmentTransaction trx = getSupportFragmentManager().beginTransaction();
		if (!fragments[index].isAdded()){
			trx.add(R.id.fragment_container, fragments[index]).commit();
		}else{
	...
	private void initView() {
		...
		fragments = new Fragment[] {homeFragment, discoveryFragment, chatHistoryFragment, myProfileFragment };
		unreadLabel = (TextView) findViewById(R.id.unread_msg_number);
		unreadAddressLable = (TextView) findViewById(R.id.unread_address_number);
		mTabs = new Button[4];
		mTabs[0] = (Button) findViewById(R.id.btn_address_list);
		mTabs[1] = (Button) findViewById(R.id.btn_discovery);
	...
	protected void onDestroy() {
		super.onDestroy();
		if(mIeetonReceiver != null){
			unregisterReceiver(mIeetonReceiver);
			mIeetonReceiver = null;
		}
		// 注销广播接收者
		try {
...
//		}
		...
		BaiduLocationHelper.ExitBaiduLocationService();
	...
	public int getUnreadAddressCountTotal() {
		...
		if (IeetonApplication.getInstance().getContactList() == null){
			return unreadAddressCountTotal;
		}
		if (IeetonApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME) != null)
			unreadAddressCountTotal = IeetonApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME)
					.getUnreadMsgCount();
		return unreadAddressCountTotal;
	}
	...
	private class NewMessageBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			// 刷新bottom bar消息未读数
			updateUnreadLabel();
			if (currentTabIndex == INPUT_CHAT_HISTORY) {
				// 当前页面如果为聊天历史页面，刷新此页面
				if (chatHistoryFragment != null) {
...
//	};
	...
	private class MyContactListener implements EMContactListener {
		...
		public void onContactAdded(List<String> usernameList) {
			// 保存增加的联系人
			Map<String, User> localUsers = IeetonApplication.getInstance().getContactList();
...
//			Log.d(TAG, username + "同意了你的好友请求");
	...
	private void saveInviteMsg(InviteMessage msg) {
		...
		// 未读数加1
		User user = IeetonApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME);
	...
	private class MyConnectionListener implements ConnectionListener {
		...
		public void onDisConnected(String errorString) {
			if (errorString != null && errorString.contains("conflict")) {
				// 显示帐号在其他设备登陆dialog
				showConflictDialog();
			} else if (currentTabIndex == INPUT_CHAT_HISTORY){
...
//					}
	...
	protected void onResume() {
		...
		if (intent != null){
			int index = intent.getIntExtra(INPUT_INDEX, INPUT_HOME);
			Utils.logd("onresume index:"+index);
			if (currentTabIndex != index){
				switchTab(index);
			}
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
            if ((System.currentTimeMillis() - mExitTime) > 2000) {
            	Utils.showToast(this, "再按一次退出程序", Toast.LENGTH_SHORT);
