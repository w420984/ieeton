...
public class CalendarCard extends View {
    ...
    private List<CustomDate> mValidDateList;	//可选日期
    ...
    private void init(Context context) {
        ...
        mCirclePaint.setColor(getResources().getColor(R.color.ieeton_color_red)); // 红色圆形
        touchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
 
        initDate();
    }

    public void setValidDate(List<CustomDate> list){
    	mValidDateList = list;
        update();
    }
    ...
    private void fillDate() {
        ...
        for (int j = 0; j < TOTAL_ROW; j++) {
            ...
            for (int i = 0; i < TOTAL_COL; i++) {
                ...
                State state = State.VALID_DAY;
                CustomDate date = null;
                // 这个月的
                if (position >= firstDayWeek
                        && position < firstDayWeek + currentMonthDays) {
                    day++;
                    date = CustomDate.modifiDayForObject(mShowDate, day);
...
//                    }
                    if (isCurrentMonth && day == monthDay ){
                    	state = State.TODAY;
                    }else if (dayIsValid(date)){
                    	state = State.VALID_DAY;
                    }else {
                    	state = State.INVALID_DAY;
                    }
                    
                    // 过去一个月
                } else if (position < firstDayWeek) {
                	date = new CustomDate(mShowDate.year,
                            mShowDate.month - 1, lastMonthDays
                            - (firstDayWeek - position - 1));
                	state = dayIsValid(date) ? State.VALID_DAY : State.INVALID_DAY;
...
//                            State.NEXT_MONTH_DAY, i, j);
                ...
                rows[j].cells[i] = new Cell(date, state, i, j);
            }
        }
    ...
    private void measureClickCell(int col, int row) {
        ...
        if (rows[row] != null) {
            ...
            date.week = col;
            if (dayIsValid(date) && mCellClickListener != null){
    ...
    class Cell {
        ...
        public void drawSelf(Canvas canvas) {
            ...
            case CURRENT_MONTH_DAY: // 当前月日期
                mTextPaint.setColor(getResources().getColor(R.color.color_invalid_day));
                break;
            ...
            case UNREACH_DAY: // 还未到的天
                mTextPaint.setColor(getResources().getColor(R.color.color_black));
                break;
            case VALID_DAY:
                mTextPaint.setColor(getResources().getColor(R.color.color_black));
                break;
    ...
    private boolean dayIsValid(CustomDate date){
    	boolean result = false;
    	if (mValidDateList == null || mValidDateList.isEmpty()){
    		result = false;
    	}else{
	    	for(CustomDate item : mValidDateList){
